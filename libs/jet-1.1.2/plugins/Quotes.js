...
JET.extend(0, "Quotes", function(utils) {
    ...
    function newSubscription(subscriptionID) {
        ...
        var subscription = { 
            ...
            start: function() {
                ...
                if (!isSubscribed) {
                    if (!(request.s || request.ch) || !atLeastOneFieldSpecified) {
                        throw "Chain or a RICs and at least one field should be specified to start a subscription";
                    }
                    ...
                    JET.publish("/MD/ControlChannel", utils.toJson(request));
                    isSubscribed = true;
                    isPaused = false;
                    isStartPaused = false;
                }
            ...
            stop: function() {
                if (isSubscribed) {
                    JET.publish("/MD/ControlChannel", utils.toJson({ "a": "unsub", "c": request.c }));
            ...
            pause: function() {
                if(isSubscribed && !isPaused) {
                   JET.publish("/MD/ControlChannel", utils.toJson({ "a": "p", "c": request.c }));
                   isPaused = true;
                }
                return this;
            },
            ...
            sendGenericMessage: function(msgName,msgPayload) {
                if(isSubscribed && !isPaused) {
                   JET.publish("/MD/ControlChannel", utils.toJson({ "a": "sendgenmsg","msgname":msgName,"msgpayload":msgPayload,"s":request.s , "c": request.c}));
                }
                return this;
            },
            ...
            resume: function() {
                if (isStartPaused) this.start();
                    ...
                    JET.publish("/MD/ControlChannel", utils.toJson({ "a": "r", "c": request.c }));
                    isPaused = false;
                }
                return this;
            },
