...
JET.extend(0, "News", function(utils) {
	...
	function init()
	{
	   JET.onLoad(function() {
		...
		JET.onLoad(function(){
			...
			JET.publish("/News/ControlChannel", utils.toJson(params));
		})
	}
	...
	function resolve(newsExpression, resolveCallback, returnOptions) {
		...
		JET.publish("/News/ControlChannel", utils.toJson(newsParams));
	}
	...
	function newSubscription(subID) {
		...
		function sendRequestForOtherHeadlines(ReqId, NumberOfHeadlines) {
				...
				JET.publish("/News/ControlChannel", utils.toJson({ a: ReqId, c: newsParams["c"], nb: NumberOfHeadlines ? NumberOfHeadlines : -1 }));
		};		
			
		function sendSimpleRequest(ReqId) {
					JET.publish("/News/ControlChannel", utils.toJson({ a: ReqId, c: newsParams["c"] }));
		};
		...
		var subscription = {
			...
			stop: function () {
				if (isSubscribed) {
					JET.publish("/News/ControlChannel", utils.toJson({ a: "unsub", c: newsParams["c"] }));
					subscriptionHandle.unsubscribe();
					isSubscribed = false;
					isStartPaused = false;
			...
			updateHighlighter: function(Pattern, HighlightObject )
			{
				...
				JET.publish("/News/ControlChannel", utils.toJson({ a: "h", p: Pattern,  h: HighlightObject, c: newsParams.c}));
			}				
		...
		var topNewsSubscription = {
            ...
            start: function() {
				...
				if (!isSubscribed)
				{
					...
					JET.publish("/News/ControlChannel", utils.toJson(request));
					isSubscribed = true;
					pausedBuffer = null;
					isStartPaused = false;
            ...
            stop: function() {
                if (isSubscribed) {
                    JET.publish("/News/ControlChannel", utils.toJson({ "a": "unsub_top", "c": request.c }));
                    subscriptionHandle.unsubscribe();
                    pausedBuffer = null;
                    isSubscribed = false;
					isStartPaused = false;
	...
	function createNewsUpdate(subID) {
		...
		var NewsUpdateSubscription = {
            ...
            stop: function() {
                if (isSubscribed) {
                    JET.publish("/News/ControlChannel", utils.toJson({ "a": "unsub_upd", "c": request.c }));
                    subscriptionHandle.unsubscribe();
                    pausedBuffer = null;
                    isSubscribed = false;
					isStartPaused = false;
