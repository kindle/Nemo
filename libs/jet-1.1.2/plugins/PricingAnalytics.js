...
//
// tracsStreamingCalculator.stopCalculator();
//
    ...
    api.createTracsCalculator = function (pluginName, tail, body) {
        return new function () {
            ...
            self.start = function () {
                ...
                self.calculatorRic = self.uuid + "_" + self.pluginName + "_" + self.calcId;
            ...
            self.updateCalculatorParameters = function (calculationTimeInterval, calculatorBody, calculatorTail) {
                ...
                var messageTypeRequest = "1";
                var headerRequest = String.fromCharCode(self.transactionLetterCode) + messageTypeRequest + self.calculationTimeInterval;
            ...
            function calculatorUpdateReceived(subscription, ric, fields, index) {
                ...
                var dataFieldsName = ric + "_" + index;

                // It's data.
                if (fields[dataFieldsName] != undefined && self.updateCallback != undefined) {

                    // Get Header information.
                    var syncLetter = fields[dataFieldsName].formatted.substring(0, 1);
                    var messageType = fields[dataFieldsName].formatted.substring(1, 2);

                    // If it's a result of an old request, don't send.
                    if (syncLetter != String.fromCharCode(self.transactionLetterCode)) return;
                    extractMessageAndSend(messageType, fields[dataFieldsName].formatted.substring(2));
                }
            ...
            function extractMessageAndSend(messageType, messsageContent) {
                ...
                if (dataReceived == undefined) return; 

                var headerContentSepIndex = dataReceived.indexOf(":");
                var resultCode = dataReceived.substring(0, headerContentSepIndex);
                var messageBody = dataReceived.substring(headerContentSepIndex + 1);

                // If plugin execution is successfull..
                if (resultCode == 200 || resultCode == 0) {

                    self.LastFullImage = dataReceived;
                    self.updateCallback(messageBody);
                    self.statusCallback("Running", resultCode);

                } else {

                    self.statusCallback("Error", resultCode);
                }
            }
